...
> ⚠️ **Windows 版说明**：我主要用 Mac，Windows 端尚未测试，由 AI 适配，有问题可以直接 issue，也可以自己改改。

> ℹ️ **win_x64 分支为 Windows 适配分支**，如需 Mac 适配，请切换到主分支：[`main`](../../tree/main)
...
- 📁 **快速定位** – 一键在资源管理器（Windows）/ Finder（macOS）中打开包所在目录或 `describe_files` 目录。
- 🚀 **分页浏览** – 支持每页 18/36/72/96 条记录，方便浏览大量数据。

## 🛠️ 系统要求

- **操作系统**：Windows x64 / macOS
...
> 缓存目录通常为：  
> - Windows: `C:\Users\<用户名>\AppData\Roaming\Unity\Asset Store-5.x\`  
> - macOS: `~/Library/Unity/Asset Store-5.x/`  
> 请根据实际情况调整。

### 3. 运行批量处理

![批处理](readme_img/batch_extract.png)
...
![整理JSON](readme_img/scan_md_file.png)

```bash
python scan_md_files.py /path/to/OUTPUT_DIR -o packages_index.json
```
...
**Windows：**
```bash
python scan_md_files.py C:\Users\你的用户名\Downloads\UnityPackage_Manifests -o packages_index.json
```
...
- 使用右侧按钮快速定位：
  - **📁 包路径** – 在资源管理器（Windows）/ Finder（macOS）中打开 `.unitypackage` 所在文件夹
  - **📋 复制路径** – 复制文件夹路径到剪贴板
  - **📄 描述文件** – 打开该包的 `describe_files` 目录
...
- **Main 分支是适配 Mac 端的，需要 Windows 端的请切换 `win_x64` 分支。**

## ⚠️ 注意事项

- 本工具**不会修改或删除**原始 `.unitypackage` 文件，所有操作均为只读。
- 由于浏览器安全策略，`file://` 协议下打开资源管理器/Finder 可能会被拦截，若拦截会自动降级为复制路径并提示。
